...
fifo_dtype = np.dtype([('k', np.int64), ('v', np.float64), ('c', np.int8)],align=True)

@njit(parallel=True,cache=True)
def new_par_dict(key_type, val_type, nothrds=4, fifo_size=1024):
    fifos = np.zeros((nothrds,nothrds,fifo_size),dtype=fifo_dtype)
...
def par_dict_setitem(state, key, val, thrd_id=None):
    ...
    #fifos[src_id,dst_id,wridx] = (key, val, 1)
...
def par_dict_delitem(state, key, thrd_id=None):
    ...
    if cap == 0:
        print("FIFO overflow!")
    wridx = wr_idx[src_id, dst_id]
    #fifos[src_id,dst_id,wridx] = (key, 0.0, 0)
    fifos['k'][src_id,dst_id,wridx] = key
